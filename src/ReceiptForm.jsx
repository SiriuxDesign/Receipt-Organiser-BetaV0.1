...
function ReceiptForm({ receipts, addReceipt, updateReceipt, deleteReceipt }) {
  ...
  const toggleAllReceipts = () => {
    ...
    const latestIndices = latestReceipts.map((_, i) => receipts.length - 3 + i)

    if (expandedReceipts.length === latestReceipts.length) {
      setExpandedReceipts([])
    } else {
      setExpandedReceipts(latestIndices)
    }
  }
  ...
  const handleFileChange = (file) => {
    if (file && file.type.startsWith('image/')) {
      ...
      reader.onloadend = () => {
        setReceipt(reader.result) // Store as base64 string
      }
  ...
  const handleAddReceipt = () => {
    ...
    const newReceipt = {
      ...
      notes,
      receipt: receipt, // Stored as base64 string
      totalAmount: totalAmount
    }
    
    if (editingIndex !== null) {
      updateReceipt(editingIndex, newReceipt)
  ...
  const handleEditReceipt = (index) => {
    ...
    setReceipt(receiptToEdit.receipt) // Load base64 string
      ...
      <div className="form-section"> {/* Added wrapper for spacing */}
        ...
        >
          ...
          <label htmlFor="receipt">
            {receipt ? `Uploaded: ${receipt.substring(0, 30)}...` : 'Drag & Drop or Click to Upload Receipt'}
          </label>
        </div>
        ...
        <div className="form-actions">
          <button className="button primary-button" onClick={handleAddReceipt}>
      ...
      {latestReceipts.length > 0 && (
        <div className="receipts-list">
          ...
          {latestReceipts.map((receipt, index) => {
            // Calculate the original index in the full receipts array
            const originalIndex = receipts.length - latestReceipts.length + index;
            return (
              <div key={originalIndex} className="saved-receipt">
                <div className="receipt-header">
                  <h3>{receipt.merchant} - {new Date(receipt.date).toLocaleDateString()}</h3>
                  <div className="icon-actions">
                    <button
                      className="icon-button expand-icon"
                      onClick={() => toggleReceiptExpansion(originalIndex)}
                    >
                      <svg viewBox="0 0 24 24" width="18" height="18">
                        <path d={expandedReceipts.includes(originalIndex) ? "M19 13H5v-2h14v2z" : "M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"} />
                      </svg>
                    </button>
                    {receipt.receipt && (
                      <a
                    ...
                    <button
                      ...
                      onClick={() => handleEditReceipt(originalIndex)}
                    >
                      <svg viewBox="0 0 24 24" width="18" height="18">
                    ...
                    <button
                      ...
                      onClick={() => handleDeleteReceipt(originalIndex)}
                    >
                      <svg viewBox="0 0 24 24" width="18" height="18">
                ...
                {expandedReceipts.includes(originalIndex) && (
                ...
                {receipt.receipt && (
                  <div className="receipt-preview">
                    <a
                      href={receipt.receipt}
                      download={`receipt_${originalIndex}.png`}
